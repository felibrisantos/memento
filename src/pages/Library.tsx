...
export default function Library() {
  ...
  const recent: RecentItem[] = (Object.entries(collection) as [Category, CollectionItem[]][])
  ...
  return (
    ...
    >
      ...
      <div className="mb-12 pt-4">
        ...
        >
          {totalItems} item{totalItems !== 1 ? 's' : ''} archived across {(Object.keys(categoryMeta) as Category[]).filter(k => collection[k]?.length > 0).length} categories
        </motion.p>
      </div>

      {/* Category Overview Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-14">
        {(Object.entries(categoryMeta) as [Category, CategoryMeta][]).map(([key, meta], i) => {
